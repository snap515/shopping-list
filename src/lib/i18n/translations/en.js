export default {
  auth: {
    login: {
      title: 'Login',
      error: 'Login failed. Check your credentials.',
      submit: 'Log in',
      noAccount: "Don't have an account? Register",
    },
    register: {
      title: 'Register',
      error: 'Registration failed. Try again.',
      submit: 'Create account',
      haveAccount: 'Already have an account? Log in',
    },
    email: {
      label: 'Email',
      placeholder: 'you@example.com',
    },
    password: {
      label: 'Password',
      placeholder: 'Enter your password',
      show: 'Show',
      hide: 'Hide',
    },
    logout: 'Log out',
    anonymous: 'anonymous',
    signedInAs: 'Signed in as',
    reset: {
      action: 'Forgot password?',
      emptyEmail: 'Enter your email to reset.',
      sent: 'Password reset email sent.',
      error: 'Could not send reset email.',
    },
    change: {
      title: 'Change password',
      placeholder: 'New password',
      submit: 'Update password',
      emptyPassword: 'Enter a new password.',
      success: 'Password updated.',
      error: 'Could not update password.',
    },
    error: {
      generic: 'Something went wrong. Please try again.',
      invalidEmail: 'Enter a valid email address.',
      weakPassword: 'Password must be at least 6 characters.',
      emailInUse: 'This email is already in use.',
      userNotFound: 'No account found for this email.',
      wrongPassword: 'Incorrect password.',
      tooManyRequests: 'Too many attempts. Try again later.',
      operationNotAllowed: 'Email/password sign-in is disabled.',
      requiresRecentLogin: 'Please log in again and retry.',
    },
  },
  lists: {
    title: 'Your lists',
    deleted: {
      title: 'List deleted',
      message: 'This list was deleted.',
      messageWithOwner: 'List "%{name}" was deleted by %{owner}.',
    },
    manage: {
      action: 'Manage',
    },
    details: {
      action: 'Settings',
    },
    create: {
      emptyName: 'Enter a list name.',
      error: 'Could not create the list.',
      permissionDenied: 'You do not have permission to create a list.',
      placeholder: 'New list name',
      submit: 'Add',
      toast: 'List "%{name}" created.',
    },
    rename: {
      emptyName: 'Enter a new name.',
      error: 'Could not rename the list.',
      placeholder: 'Rename list',
      action: 'Rename',
      toast: 'List renamed to "%{name}".',
    },
    delete: {
      error: 'Could not delete the list.',
      title: 'Delete list?',
      message: 'This will remove the list "%{name}" for everyone.',
      action: 'Delete list',
      toast: 'List "%{name}" deleted.',
    },
    membersCount: 'Members',
  },
  listDetails: {
    title: 'Shopping list',
    leave: {
      title: 'Leave list?',
      message: 'You will lose access to this list.',
      action: 'Leave list',
      error: 'Could not leave the list.',
      toastTitle: 'Left list',
      toast: 'You left the list "%{name}".',
    },
    actions: {
      title: 'List actions',
      clearPurchased: 'Clear purchased',
      clearAll: 'Clear all',
      clearPurchasedTitle: 'Clear purchased items?',
      clearPurchasedMessage: 'This will remove all purchased items.',
      clearAllTitle: 'Clear the list?',
      clearAllMessage: 'This will remove all items from the list.',
      clearPurchasedError: 'Could not clear purchased items.',
      clearAllError: 'Could not clear the list.',
      clearPurchasedToast: 'Purchased items cleared.',
      clearAllToast: 'List cleared.',
    },
  },
  listInfo: {
    title: 'List settings',
    membersTitle: 'Shared with',
    membersEmpty: 'No members yet.',
    ownerBadge: 'Owner',
    youSuffix: '(you)',
    inviteTitle: 'Invite',
    renameTitle: 'Rename list',
  },
  items: {
    meta: {
      createdBy: 'Created by:',
      doneBy: 'Done by:',
    },
    add: {
      empty: 'Enter an item.',
      error: 'Could not add the item.',
      placeholder: 'Add an item',
      submit: 'Add',
    },
    rename: {
      action: 'Rename item',
      save: 'Save item',
      cancel: 'Cancel rename',
      empty: 'Enter a new item name.',
      error: 'Could not rename the item.',
    },
    toggle: {
      error: 'Could not update the item.',
    },
    delete: {
      error: 'Could not delete the item.',
    },
  },
  invites: {
    create: {
      empty: 'Enter an email to invite.',
      error: 'Could not send the invite.',
      alreadyMember: 'This user is already in the list.',
      alreadyPending: 'This invite is already pending.',
      permissionDenied: 'You do not have permission to send invites.',
      placeholder: 'Invite by email',
      submit: 'Invite',
      sent: 'Invite sent to %{email}.',
    },
    title: 'Invites',
    empty: 'No invites yet.',
    list: 'List:',
    from: 'From:',
    accept: 'Accept',
    decline: 'Decline',
    acceptError: 'Could not accept the invite.',
    declineError: 'Could not decline the invite.',
    acceptSuccess: 'You joined "%{name}".',
  },
  settings: {
    title: 'Settings',
    language: {
      title: 'Language',
      ru: 'Russian',
      de: 'German',
      en: 'English',
    },
    theme: {
      title: 'Theme',
      light: 'Light',
      dark: 'Dark',
    },
  },
  tabs: {
    lists: 'Lists',
    invites: 'Invites',
    templates: 'Templates',
    settings: 'Settings',
  },
  templates: {
    title: 'Templates',
    recipes: {
      title: 'Recipes',
      subtitle: 'Ready-made recipes',
    },
    addTemplate: 'Add template',
    itemsCount: '%{count} items',
    createTitle: 'New template',
    detailsTitle: 'Template',
    addSet: 'Add set',
    createSetTitle: 'New set',
    setDetailsTitle: 'Set',
    setsEmpty: 'No sets yet.',
    deleteSetTitle: 'Delete set?',
    deleteSetMessage: 'Set "%{name}" will be deleted.',
    deleteSetToast: 'Set "%{name}" deleted.',
    deleteSetError: 'Could not delete the set.',
    renameSetSuccess: 'Set renamed to "%{name}".',
    renameSetError: 'Could not rename the set.',
    saveSet: 'Save set',
    savedSetToast: 'Set "%{name}" saved.',
    saveSetError: 'Could not save the set.',
    setNotFound: 'Set not found.',
    namePlaceholder: 'Template name',
    ingredientPlaceholder: 'Add item',
    addIngredient: 'Add',
    saveTemplate: 'Save template',
    nameRequired: 'Enter a template name.',
    itemsRequired: 'Add at least one item.',
    saveError: 'Could not save the template.',
    savedToast: 'Template "%{name}" saved.',
    renameSuccess: 'Template renamed to "%{name}".',
    renameError: 'Could not rename the template.',
    itemsEmpty: 'No items yet.',
    itemRequired: 'Item name is required.',
    itemUpdated: 'Item updated.',
    itemDeleted: 'Item removed.',
    itemUpdateError: 'Could not update the item.',
    deleteItemTitle: 'Remove item?',
    deleteItemConfirm: 'This item will be removed from the template.',
    deleteTitle: 'Delete template?',
    deleteMessage: 'Template "%{name}" will be deleted.',
    deleteToast: 'Template "%{name}" deleted.',
    deleteError: 'Could not delete the template.',
    notFound: 'Template not found.',
    addToList: 'Add to list',
    addToNewList: 'Add to new list',
    addToExisting: 'Add to existing list',
    selectList: 'Choose a list',
    noLists: 'No lists yet.',
    addedToExistingToast: 'Items added to "%{name}".',
    addToListError: 'Could not add items to the list.',
    createListTitle: 'Create list',
    createListPlaceholder: 'List name',
    createListAction: 'Create list',
    createListNameRequired: 'Enter a list name.',
    toastAdded: 'List created from "%{name}".',
    toastError: 'Could not create list.',
  },
  recipes: {
    title: 'Recipes',
    detailsTitle: 'Recipe',
    createTitle: 'New recipe',
    empty: 'No recipes yet.',
    count: '%{count} recipes',
    addRecipe: 'Add recipe',
    deleteTitle: 'Delete recipe?',
    deleteMessage: 'Recipe "%{name}" will be deleted.',
    deleteToast: 'Recipe "%{name}" deleted.',
    deleteError: 'Could not delete the recipe.',
    namePlaceholder: 'Recipe name',
    ingredientPlaceholder: 'Add ingredient',
    addIngredient: 'Add',
    saveRecipe: 'Save recipe',
    nameRequired: 'Enter a recipe name.',
    itemsRequired: 'Add at least one ingredient.',
    saveError: 'Could not save the recipe.',
    savedToast: 'Recipe "%{name}" saved.',
    renameSuccess: 'Recipe renamed to "%{name}".',
    renameError: 'Could not rename the recipe.',
    itemsEmpty: 'No ingredients yet.',
    itemRequired: 'Ingredient name is required.',
    itemUpdated: 'Ingredient updated.',
    itemDeleted: 'Ingredient removed.',
    itemUpdateError: 'Could not update the ingredient.',
    deleteItemTitle: 'Remove ingredient?',
    deleteItemConfirm: 'This ingredient will be removed from the recipe.',
    itemsCount: '%{count} items',
    addToList: 'Add to new list',
    addToExisting: 'Add to existing list',
    selectList: 'Choose a list',
    noLists: 'No lists yet.',
    createListTitle: 'Create list',
    createListPlaceholder: 'List name',
    createListAction: 'Create list',
    createListNameRequired: 'Enter a list name.',
    notFound: 'Recipe not found.',
    toastAdded: 'List created from "%{name}".',
    addedToExistingToast: 'Ingredients added to "%{name}".',
    toastError: 'Could not create list.',
  },
  common: {
    delete: 'Delete',
    cancel: 'Cancel',
    save: 'Save',
    ok: 'OK',
  },
};
